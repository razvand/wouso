...
def main():
    ...
    for r in Race.objects.all():
        num_players = len(Player.objects.filter(race=r, user__date_joined__lte=datetime.datetime(2015,3,1,0,0)))
        data[r] = {
    ...
    for q in active_quizzes:
        ...
        for uq in user_quizzes:
            ...
            try:
                ...
                if user.date_joined > datetime.datetime(2015,4,3,0,0):
                    continue
            except Exception, e:
                continue
            if q.id == 22:
    ...
    for a in access_activities:
        if a.timestamp > datetime.datetime(2015,4,3,0,0):
            continue
        player = a.user_from
